  ...
  const getItems = (value) => {
    return (
      value.split(',').map((i, index) => (
        <li className="multiselect__item" key={i+index} onClick={() => handleFilter(i)}>
          <span className="multiselect__icon">
            {items.includes(i) &&
            <svg viewBox="0 0 468.293 468.293">
              <circle cx="234.146" cy="234.146" r="234.146"/>
              <polygon fill="#fff" points="357.52,110.145 191.995,275.67 110.773,194.451 69.534,235.684 191.995,358.148
                398.759,151.378 "/>
            </svg>}
          </span>
          {i}
